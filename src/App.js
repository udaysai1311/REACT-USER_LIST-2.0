...
class App extends Component {
  ...
  deleteUser = uniqueNo => {
    ...
    const filterUser = userDetailsList.filter(
      eachUser => eachUser.uniqueNo !== uniqueNo,
    )
    this.setState({userDetailsList: filterUser})
  }

  render() {
    const {searchInput, userDetailsList} = this.state
    const searchResult = userDetailsList.filter(
      eachUser => eachUser.name.includes(searchInput), // filter returns the element which satisfies(true) the condition.
    )
    return (
      <div className="app-container">
        <h1 className="title">Users List</h1>
        <input type="search" onChange={this.onChangeInputEvent} />
